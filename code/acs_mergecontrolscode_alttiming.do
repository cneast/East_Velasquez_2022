...
merge m:1 statefip `o' year using $ENFOR/final_bartik_acs_`o'.dta 
tab year _merge
...
keep if (_merge==3 | year==2000) // didnt create this data for 2000 so keep 2000 sample anyway
drop _merge  
...
drop _merge
drop *_march *_jan
for any jail287g task287g state287g SC: rename X_frac X_frac_lastyr
replace year=year+1
...
drop _merge
drop *_march *_jan
for any jail287g task287g state287g SC: rename X_frac X_frac_nextyr
replace year=year-1
